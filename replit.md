...
- **Match Management**: Includes a visual court component, queuing system, and a deterministic Smart Match Engine v4 with gender-aware logic and 9-tier grade-based scoring, enhanced by the BPG Competitive Balance Engine and Session Fairness Command Center. v2 upgrades: capped deficit penalty, match quality floor hard filters, adaptive candidate limit, strengthened partner penalty, configurable gender quota, rebalanced priority, Match Quality Score factor, session phase logic (EARLY/MID/LATE weight multipliers), anti-dominance tracking, match closeness memory, court assignment optimization (sort by qualityScore), grade rank cache. v3 upgrades: fully configurable engine settings via admin UI, preset system (Casual/Balanced/Competitive), preview simulation. v4 upgrades: 40% female-only cap (`femaleOnlyMaxRatio`) with forced mixed minimum (`mixedMinRatio`), strong male+female bonus (`strongMaleFemaleBonus: +25`), no-strong-male penalty (`noStrongMaleFemalePenalty: -40`), mixed match bonus (`mixedMatchBonus: +20`), same-gender penalty (`sameGenderMatchPenalty: -10`), male rotation scaling (`maleRotationScaling: -15`), consecutive opponent hard block (`consecutiveOpponentBlock: 2`), soft opponent penalty (`softOpponentPenalty: -25`), 4-player group repeat penalty (`groupRepeatPenalty: -50`), hard grade spread filter (`hardGradeSpreadLimit: 4`), doubles team avg diff filter (`teamAvgDiffLimit: 3`). All settings configurable via admin UI at `/admin/match-engine-settings`.
