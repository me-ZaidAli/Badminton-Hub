...
- **Session Financial Command Center**: Real-time financial overview per session with expected vs actual revenue comparison, per-player financial actions, payment reminders, and credit issuance. Player breakdown now includes CONFIRMED + WAITING + INVITED players (sorted CONFIRMED → INVITED → WAITING by position) with status badges, summary counts (`waitingCount`/`invitedCount`); revenue figures still derive from confirmed-only. Admins can add players from the dashboard via `POST /api/sessions/:id/admin-add-player` (validates profile exists, belongs to session.clubId, membership APPROVED, no duplicate; auto-routes to CONFIRMED or WAITING based on capacity; fee = session → club → membership-plan), promote from waiting via the hardened `POST /api/sessions/:sessionId/promote-waiting` (validates session, WAITING state, and capacity), and remove via the existing DELETE signups route. Add Player dialog reuses `/api/clubs/:clubId/members` (endpoint now permits MANAGE_CLUB OR MANAGE_SESSIONS) with explicit error/retry UI. All mutations invalidate both `signups` and `financial-overview` query keys.
...
- **Session Banner, Formatting & Useful Links**: Sessions support an optional admin-set coloured banner (red/amber/blue/green/purple/pink) with a formatted message (max 2000 chars) and an explicit On/Off toggle in the create + edit forms — when off, both `bannerMessage` and `bannerColor` are persisted as `null`. Banner text now supports lightweight inline formatting via the in-house `FormattedBannerText` renderer in `client/src/components/SessionViews.tsx`: `**bold**`, `- ` / `* ` / `• ` bullets (rendered as `<ul>`), `# ` / `## ` headings, and blank-line spacing — mobile-friendly with `break-words`. Session description (`sessionDetails`) appears in an amber-bordered highlighted box on the timeline card, modal, and SessionDetail page. Sessions also support an admin-managed `customLinks` array (up to 10 `{title, url}` pairs, e.g. WhatsApp group, Google Maps, club rules) added/removed in both create + edit forms; rendered as a "Useful Links" panel via the reusable `UsefulLinks` component on the timeline card, `SessionDetailsModal`, and `SessionDetail` page (mobile: 1 col, sm+: 2 cols, opens in new tab with `rel=noopener noreferrer`). Schema additions: `bannerMessage`, `bannerColor`, and `customLinks jsonb default '[]'` on the `sessions` table. `insertSessionSchema` validates banner length (≤2000), colour enum, and customLinks (max 10, title 1–60 chars, url ≤500). `PATCH /api/sessions/:id` and the recurring `apply-to-series` endpoint both validate + persist these fields and enforce http/https-only URLs server-side (rejects javascript:/data:/etc.).
