...
- **Payment flow — self-declared bank-transfer details (no image upload)**: All 3 BSL payment endpoints (`POST /api/bsl/clubs/:id/payment-proof`, `POST /api/bsl/players/:id/payment-proof`, `POST /api/bsl/wallet/topup`) accept JSON `{paymentAmountPence, paymentDate (YYYY-MM-DD), payerAccountName (≤120 chars)}` and write to `bsl_clubs.payment{AmountPence,Date}/payerAccountName` / `bsl_players.payment{AmountPence,Date}/payerAccountName` / `bsl_wallet_transactions.payment{Date}/payerAccountName`. Multer/Object-Storage upload removed from these routes (admin media route `POST /api/bsl/admin/media` still uses upload). Legacy `paymentProofUrl`/`proofUrl` columns retained for old rows but never written. Admin queues (`/bsl/admin/verify`, `/bsl/admin/payments`) render an inline `PayDetails`/`Row pay` pill (`amount · date / payer name`) — admin cross-checks against the bank statement before clicking Approve.
...
### Tournaments — legacy pair admin dissolve (May 2026)
Pairs created before the per-category system live as pair-requests with `categoryId IS NULL` + both registrations PAIR/partnerId. They blocked both players from being re-paired in any new category and were invisible to admins (PairsTab now groups by category). Fix: `GET /api/tournaments/:id/pairs` now returns `categoryId` on each pair (from the matching pair-request, NULL = legacy). PairsTab shows an admin-only amber "Legacy tournament-wide pairs" panel listing them with a Dissolve button. New endpoint `POST /api/tournaments/:id/admin-dissolve-pair {userId}` (tournament-admin only) clones the `/unpair` logic: resets BOTH registrations to INDIVIDUAL/null partnerId, marks the ACCEPTED pair-request DISSOLVED, and tears down any stale team rows + group/match/standings for that pair across all categories. Hook `useAdminDissolvePair` invalidates pairs / registrations / player-pool / teams-by-category / my-categories. AdminTab Create Pair candidate list now includes EVERY approved player (not just `registrationType === "INDIVIDUAL"`) — the server's per-category unique index (409) is the source of truth for who's already paired in a given category, so players paired in one category still surface as candidates for others.
