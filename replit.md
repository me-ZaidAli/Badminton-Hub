...
- **Match Management**: Includes a visual court component, queuing system, and a deterministic Smart Match Engine v6 with multi-mode matchmaking (ADVANCED/HYBRID/ROTATION), gender-aware logic, 9-tier grade-based scoring, enhanced by the BPG Competitive Balance Engine and Session Fairness Command Center. It includes features like capped deficit penalty, match quality floor filters, adaptive candidate limit, opponent cooldown, and various configuration options via an admin UI. **Engine Control Panel settings are fully wired**: `scoreMatch`, `generateNextMatch`, and `generateSinglesMatches` all read from `MatchEngineSettings` (via `engineConfig`) for spread/partner/opponent weights, grade spread limits, team avg diff limits, candidate window size, priority bonus, and strong-weak pairing bonus. No hardcoded scoring values remain in the core match generation path. **Match Type Distribution steering** actively steers match generation towards configured `maleOnlyTargetRatio`/`femaleOnlyTargetRatio`/`mixedTargetRatio` targets using running type counts. Each club has its own independent engine configuration stored in `clubs.matchEngineSettings`. The Engine Control Panel always shows a club selector so admins/super admins can switch between clubs.
...
- **Tournaments Module**: Full tournament management with esports-style UI, supporting various tournament types, registration, group/knockout brackets, score submission, standings, and an extensive admin panel. Includes a dual entry fee system and a prize management system. GROUP_KNOCKOUT format supports a proper group stage (round robin within groups) with admin ability to manually add unlimited group matches via "Add Match" per group, followed by knockout stage where top 2 from each group advance. API: `POST /api/tournament-categories/:id/add-group-match`. GROUP_KNOCKOUT uses a 3-stage flow: Group Stage (round robin, points-based) → Semi-Finals (top 2 per group play round robin against all qualifiers, points-based) → Final (top 2 from semi-finals). Semi-final matches use `groupNumber=100`, final uses `round=200`. Semi-final standings shown separately with amber styling. Final generation requires all semi-final matches to be completed first. Categories can be deleted with cascading cleanup of all related data (matches, standings, teams, groups, prizes, stats).
