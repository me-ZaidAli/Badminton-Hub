...
-   **Internal Messaging System**: Users can send internal messages to each other by email address. Inbox page with inbox/sent tabs, compose dialog, reply, delete, bulk select, mark all read, and search. Backend supports send (by recipientId or recipientEmail), inbox, sent, unread count, mark read, mark all read, and delete.
