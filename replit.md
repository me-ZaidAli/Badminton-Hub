...
- **Club-Restricted Wallet System**: Dedicated Wallet Management page (`/super-admin/wallets`) with global or club-restricted credit wallets per user. Wallets are **automatically created** whenever credits are issued (rewards, referrals, session cancellations, recognition cards, trial approvals, etc.) via the `ensureUserWallet` helper. Admins can also manually create wallets, add/remove funds, assign club permissions (multi-select), and view full transaction audit logs. A "Sync Credit Wallets" migration button creates wallets for all existing users with positive credit balances. Supports multiple wallets per user with low-balance alerts. Deductions validate against club permissions, and balances are reflected across the app via `/api/my-wallets` and `/api/my-wallets/eligible/:clubId` endpoints.
