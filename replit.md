...
- **Club T-Shirt System**: Club-scoped t-shirt management with player requests, admin management, and batch ordering. Players see a club selector on `/tshirt` page (only clubs they belong to that have `providesClubTShirts: true`). T-shirts and requests are filtered per selected club. 3 models available: Chaotica (`@assets/image_1775147617249.png`), Slasher (`@assets/image_1775147642411.png`), Striker (`@assets/image_1775250192402.png`). Admins manage via `/admin/tshirts` with T-Shirt Manager and Batch Manager. Tables: `tshirts`, `tshirt_requests`, `tshirt_batches`. Collection flow: not_ready → ready → player_confirmed → collected. All state changes logged to `admin_audit_logs` and trigger notifications.
