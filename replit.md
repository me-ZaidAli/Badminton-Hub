...
- **Birmingham Super League (BSL)**: Esports-style competition module at `/bsl`. Locked palette derived from BSL poster (deep navy bg `222 50% 6%`, electric cyan `195 100% 60%`, gold/amber `42 95% 55%`) — no other hues allowed. Routes: `/bsl` (LeagueMode hub), `/bsl/register-club` (7-step wizard), `/bsl/join` (invite-code flow), `/bsl/wallet` (balance + top-up + tx history), `/bsl/match/:id` (6-rubber scoreboard with admin edit), `/bsl/admin/verify` (approve clubs/players/top-ups, OWNER/ADMIN), `/bsl/admin/fixtures` (HTML5 drag-drop court assignment, OWNER/ADMIN). Backend in `server/bsl-routes.ts` (registered from `server/routes.ts`); schema at `shared/schema.ts` lines ~3050-3211 (5 enums + 8 tables: bslLeagues/Clubs/Teams/Players/LeagueDays/Fixtures/Rubbers/WalletTransactions). Payment flow is bank-transfer + uploaded proof to `/uploads/bsl/`, admin-approved. League day singleton (`bslLeagues` id=1) seeded with default bank details, £500 club fee, £25 player fee. Components in `client/src/pages/bsl/components/` (BSLPalette, BSLBackground, GlowPanel, ActionButton, CountdownTimer, StatTile, MatchCard, LeaderRow). All motion via framer-motion (entrance stagger, hover lift, pulsing accent glows, animated score counters, drag-drop spring physics).
