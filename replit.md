...
- **Matchmaking**: Smart Match Engine v6.1 (gender-aware, 9-tier grade scoring) in `server/matchEngine.ts` + `server/adaptiveFairnessAI.ts`; defaults in `shared/matchEngineSettings.ts`. v6.1 (May 2026) added persistent `recentGroups` history across per-call invocations, O(N⁴)→capped group generation (1500), Map-based AI sort, fixed fallback relaxation, fixed mixed-game male lockout, and tightened defaults (gradeSpread 5→3, hardSpread 4→5, teamAvgDiff 3→1.5).
