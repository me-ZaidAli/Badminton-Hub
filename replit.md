...
- **Match Management**: Includes a visual court component, queuing system, and a deterministic Smart Match Engine v2 with gender-aware logic and 9-tier grade-based scoring, enhanced by the BPG Competitive Balance Engine and Session Fairness Command Center. v2 upgrades: capped deficit penalty (-100/-170/-210), match quality floor hard filters, adaptive candidate limit (120/180/240), strengthened partner penalty (-25/-50/-80), configurable gender quota (default 70%), rebalanced priority (+150/+80), Match Quality Score factor, session phase logic (EARLY/MID/LATE weight multipliers), anti-dominance tracking, match closeness memory, court assignment optimization (sort by qualityScore), grade rank cache. New `EngineSettings` type with `femaleQuotaRatio`, `matchScoreHistory`, `totalSessionMatches`. `MatchResult` includes optional `qualityScore`. All backward compatible.
