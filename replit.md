...
-   **Attendance Analytics Dashboard**: Comprehensive attendance analytics with KPI cards (total sessions, attendances, unique members, avg attendance, growth, no-show rate), clickable drill-down modals, High Attendance bar chart (top 10/20 members), Attendance Distribution frequency buckets, Attendance Over Time line chart with overlays, and Session Performance analysis by type. All views are RBAC-filtered (Admin sees assigned clubs, OWNER sees all). Includes member detail modals with engagement scores, in-app messaging, admin notes, and audit logging.
-   **Inactive Members Management**: Dashboard identifying members with configurable inactivity thresholds (30/60/90+ days, custom). Features re-engagement messaging, admin notes, 3-day scheduled account deletion with countdown, internal notifications, cancellation flow, OWNER-only permanent deletion, and full audit logging via `admin_audit_logs` table. Users table extended with `deletion_scheduled_at`, `deletion_scheduled_by`, `deletion_reason` fields.
-   **Admin Player Rewards Viewer**: On the Rewards page, admins and super admins see a searchable player dropdown to look up any player by name or email and view their full rewards data (referrals, attendance, anniversary, birthday). RBAC-scoped: club admins only see players and rewards within their managed clubs; OWNERs see all. Points and badges tabs are hidden when viewing another player.
