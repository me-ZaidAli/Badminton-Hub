...
- **Financial Dashboard**: Accessible to OWNER, ADMIN, and club owners/admins at `/admin/financials`. Shows total revenue, collected payments, outstanding fees, and collection rate. Features session-grouped and player-grouped views with expandable rows showing individual payment records. Admins can mark payments as PAID/UNPAID and edit fees inline. Non-OWNER users see only data from their accessible clubs (owned or managed). Endpoints: GET `/api/admin/financial-summary`, PATCH `/api/sessions/:sessionId/signups/:signupId/payment`, PATCH `/api/admin/signups/:signupId/fee`. All endpoints enforce club-scoped authorization.
