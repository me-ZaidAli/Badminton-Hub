...
- **Membership System**: Comprehensive club-based membership system with membership plans, requests, approvals, and proration. Tables: `membership_plans` (club-scoped plans with annual price, default session fee), `club_memberships` (per-user-per-club, one active per club, status: PENDING/ACTIVE/EXPIRING/EXPIRED/CANCELLED), `membership_requests` (request flow with approval/rejection). Membership year: Sept 1 – Aug 31 with proration (remaining months/12 × annual price). Prominent expiry counter (>60 days green, 30-60 amber, <30 red, expired grey). Fee priority: membership default → manual override → credit deduction. RBAC: MANAGE_MEMBERSHIPS action for OWNER/ADMIN. User pages: `/memberships` (view/request/cancel/order merch). Admin pages: `/admin/membership-board` (filters, bulk actions, request queue, plan management). Endpoints: GET/POST/PATCH/DELETE membership plans, requests, memberships, merchandise, and orders. Session fee resolution: GET `/api/session-fee/:sessionId/:userId`.
- **Merchandise System**: Clubs create merchandise items with optional size selection and membership inclusion. Tables: `merchandise` (club-scoped items), `merchandise_orders` (user orders with size/quantity). Included-in-membership items are free for active members.
