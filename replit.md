...
- **Advanced Analytics Dashboard**: Power BI-level business intelligence system at `/dashboard/analytics`. Features: 10 KPI cards, global filter system (date range, club, session, time-of-day, weekday), interactive charts (attendance trends, revenue over time daily/weekly/monthly, most popular sessions, most profitable sessions, time-of-day attendance, weekday performance, club comparison, capacity utilization histogram, player retention pie chart, attendance frequency distribution, no-show analysis, seasonality trends, player churn & loyalty), sortable/searchable session performance table with pagination, smart alerts (attendance drops, revenue declines, low/over capacity), demand & scheduling optimization suggestions, AI insights & analysis with GPT-4o-mini (full report generation + natural language data queries), CSV export. Routes: `/api/dashboard/analytics` (GET), `/api/dashboard/analytics/ai-insights` (POST). Access: Admin/Owner only, premium-gated in sidebar. Key files: `client/src/pages/AnalyticsDashboard.tsx`, `server/routes.ts`.
