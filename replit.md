...
- **Financial Intelligence System**: Multi-view financial dashboard with Smart Insights, credit management, donation system, and per-session invoice tracking. Includes a unified wallet/credit system where the wallet is the single source of truth for all credit balances.
...
- **Tournaments Module**: Full tournament management with esports-style UI, supporting various tournament types, registration, group/knockout brackets, score submission, standings, and an extensive admin panel. Includes dual entry fee and prize management systems.
...
- **Deals & Offers System**: Premium mobile-first experience at `/deals` with category discovery page (2-column grid with gradient cards, floating emojis, deal counts), filtered deals view with sticky search bar, pill-style filter chips (All, 10%+, 20%+, 50%+, Expiring Soon, New), featured deal card with countdown timer, glassmorphism card design with card-flip animation for details, save/favorite functionality, copy-code feedback with "Copied ✅", smooth framer-motion animations throughout. Categories: Food, Fitness, Services, Equipment, Wellness, Travel, Other. Admin management tab with category selection. `discount_codes` schema includes `category` column. Per-club scoping with assignment system.
