...
A player registers ONCE for a tournament, then joins individual categories with a different partner per category. Schema: `tournament_pair_requests.category_id` (NULL = legacy tournament-wide pair). Endpoints (`server/tournamentRoutes.ts`): `POST /api/tournaments/:id/pair-requests` accepts optional `categoryId`; `PATCH /api/tournament-pair-requests/:id` on ACCEPT materialises a `tournament_teams` row in that category (replacing any solo placeholder) instead of mutating registrations. New: `POST /api/tournament-categories/:id/join-solo` (creates a team with player1 only — for singles, or "looking for partner" placeholder), `POST /api/tournament-categories/:id/leave` (cascades matches/standings/group_pairs + dissolves ACCEPTED per-category requests), `GET /api/tournaments/:id/my-categories` (per-category status + pending requests). UI: new "My Categories" subtab in `client/src/pages/TournamentDetail.tsx` (`MyCategoriesTab`) — per-category join/leave + partner picker dialog. Hooks `useMyTournamentCategories`, `useJoinCategorySolo`, `useLeaveCategory` in `client/src/hooks/use-tournaments.ts`. Legacy single-category registration flow (`registrationType`/`partnerId` on `tournament_registrations`) preserved for back-compat — only triggered when pair-request has NULL categoryId.
