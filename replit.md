...
- **Match Management**: Includes a visual court component, queuing system, and a deterministic Smart Match Engine v5 with gender-aware logic and 9-tier grade-based scoring, enhanced by the BPG Competitive Balance Engine and Session Fairness Command Center. v2 upgrades: capped deficit penalty, match quality floor hard filters, adaptive candidate limit, strengthened partner penalty, rebalanced priority, Match Quality Score factor, session phase logic (EARLY/MID/LATE weight multipliers), anti-dominance tracking, match closeness memory, court assignment optimization (sort by qualityScore), grade rank cache. v3 upgrades: fully configurable engine settings via admin UI, preset system (Casual/Balanced/Competitive), preview simulation. v4 upgrades: strong male+female bonus, no-strong-male penalty, consecutive opponent hard block, soft opponent penalty, 4-player group repeat penalty, hard grade spread filter, doubles team avg diff filter. v5 upgrades: replaced hard gender slot system with dynamic 3-way soft ratio system — `maleOnlyTargetRatio` (50%), `femaleOnlyTargetRatio` (30%), `mixedTargetRatio` (20%) with `ratioBoostBonus` (+25 for underrepresented types) and `ratioOverPenalty` (-20 for overrepresented types). Removed `sameGenderMatchPenalty` global bias — distribution controlled only by ratio system. Added `isMaleOnlyMatch` helper and `scoreMatchTypeRatio` function. Real-time match type counters (maleOnly, femaleOnly, mixed) track distribution during generation. All settings configurable via admin UI at `/admin/match-engine-settings`.
