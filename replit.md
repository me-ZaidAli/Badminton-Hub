...
- **Supplier Sheet PDF Privacy**: Merchandise supplier order sheets contain ZERO personal customer data — no names, no emails, no phone numbers. The "Customer" column has been replaced with an "Order Ref" column showing only the internal order number (e.g. `#1234`). User identifying fields are no longer selected from the database, no longer passed to the PDF generator, and no longer rendered. Suppliers receive only product/variant/qty/back-name (the back-name being printed ON the garment, not customer info).
- **Quick Session Card Actions**: The expanded session card on the Timeline view exposes one-click actions so users no longer need to drill into the session detail page. Signed-up players see a red "Withdraw" button alongside the "Joined"/"On Waitlist" badge (with a confirmation prompt that warns the spot may be reassigned to someone on the waitlist). Admins, organisers, super admins and club owners see a visible red "Delete" button in the admin controls bar (with a confirmation prompt warning that signups, matches and finances will be removed) — the dropdown-menu Delete entry is also retained.
