...
- **Juniors Page**: Consolidated parent-facing page at `/juniors` with Hero banner and 5 tabbed sections: **My Children** (CRUD with rich profile cards showing skill gauge, attendance, effort/coach ratings, awards, videos), **Performance** (embedded inline skill dashboard with child selector — profile header, circular gauge, skill categories, rankings, achievements, videos all inline without separate navigation), **Sessions** (JUNIORS_ONLY sessions listing), **Fees** (3 plans: £15 group, £25 1-to-1, £8 matches), **About** (What We Do + Safeguarding). URL params `?tab=&child=` supported for deep-linking. `/juniors/dashboard/:userId` redirects to the Performance tab. Sidebar item with Baby icon in activity group.
...
- **Junior Skill Development & Tracking System**: Full skill tracking system for junior players with 11 categories (Game Rules, Warm Up, Physical, Footwork, Positioning, Service, Attack, Defense, Strategic Shot, Psychology, Sync) and 66 individual skills. Mobile-first dashboard at `/juniors/dashboard/:userId` with profile card (circular gauge, attendance %, effort/coach star ratings), collapsible skill category cards with progress bars, admin bottom-sheet editor (slider 0-100, priority toggle, coach comments), Rankings tab (top 10, movement arrows), Achievements grid (15 badges including per-category mastery), Videos tab (YouTube embeds with coach comments). RBAC: parents see own children read-only, admins can edit. DB tables: `junior_skill_categories`, `junior_skills`, `junior_profiles`, `junior_skill_progress`, `junior_achievements`, `junior_videos`, `junior_rankings`. Auto-seeded on startup via `server/juniorSkillsSeed.ts`.
