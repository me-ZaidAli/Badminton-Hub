...
### UI/UX Decisions
The UI incorporates a privacy-enhanced public view, a modern player profile dashboard with drill-down metrics, and an animated anniversary countdown. The system offers a comprehensive Premium Theme System with 20 themes across 5 tiers: Standard (Light, Dark, Premium Gold, Ultra-Premium, Green Glow), Accessibility (Sepia, Migraine, High Contrast, Grayscale), Premium (Obsidian Gold, Platinum Ice, Emerald Performance, Carbon Titanium, AMOLED Black), Elite (Sapphire Velocity, Crimson Prestige, Arctic Blue — requires Top 10 ranking), Signature (Royal Amethyst, Sunset Copper — requires #1 ranking), and Ultra Exclusive (Midnight Neon — Black Card access only). A dedicated Theme Gallery page at `/themes` provides filterable preview cards with gradient samples, chart color previews, button style previews, and live instant switching. Theme data is stored in `use-theme.ts` with CSS classes in `index.css`. Ranking-unlocked themes are verified via `GET /api/user/available-themes`. Black Card access is managed via `users.blackCardAccess` field and `PATCH /api/admin/users/:id/black-card`.
