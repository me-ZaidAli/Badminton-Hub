...
- **Match Management**: Includes a visual court component, queuing system, deterministic Smart Match Engine (social/competitive) with 9-tier grade-based scoring (C3→A1), state invariants (player in one match only), atomic player movement, validation pipeline, scoring decision logging, and controls for live match cancellation.
