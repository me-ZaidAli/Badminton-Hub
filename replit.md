...
- **Club T-Shirt System**: Full t-shirt management with player requests, admin management, and batch ordering. Players can request t-shirts via `/tshirt` page, view status on dashboard card, and confirm collection. Admins manage t-shirts via `/admin/tshirts` with T-Shirt Manager (create/edit records, mark ready, confirm collection) and Batch Manager (aggregates pending requests, creates batches when ≥10 requests). Tables: `tshirts` (linked to playerProfiles), `tshirt_requests`, `tshirt_batches`. Collection status flow: not_ready → ready → player_confirmed → collected. All state changes logged to `admin_audit_logs` and trigger notifications.
