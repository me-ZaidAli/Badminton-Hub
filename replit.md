...
- **Match Management**: Includes a visual court component, queuing system, and a deterministic Smart Match Engine v6 with multi-mode matchmaking (ADVANCED/HYBRID/ROTATION), gender-aware logic, and 9-tier grade-based scoring, enhanced by the BPG Competitive Balance Engine and Session Fairness Command Center. v2 upgrades: capped deficit penalty, match quality floor hard filters, adaptive candidate limit, strengthened partner penalty, rebalanced priority, Match Quality Score factor, session phase logic (EARLY/MID/LATE weight multipliers), anti-dominance tracking, match closeness memory, court assignment optimization (sort by qualityScore), grade rank cache. v3 upgrades: fully configurable engine settings via admin UI, preset system (Casual/Balanced/Competitive/Quick Rotation), preview simulation. v4 upgrades: strong male+female bonus, no-strong-male penalty, soft opponent penalty, 4-player group repeat penalty, hard grade spread filter, doubles team avg diff filter. v5 upgrades: replaced hard gender slot system with dynamic 3-way soft ratio system — `maleOnlyTargetRatio` (50%), `femaleOnlyTargetRatio` (30%), `mixedTargetRatio` (20%). v6 upgrades: slot-based match type distribution via `computeMatchSlots()`, opponent cooldown system with windowed history (`opponentCooldownWindow`/`opponentCooldownThreshold`), mixed match player hard limits (`mixedMatchPlayerLimit`/`mixedMatchPlayerWindow`). v6 multi-mode system: ADVANCED mode (full optimisation engine with all scoring, slot-based distribution, strict cooldowns, quality filters, gender balance), HYBRID mode (rotation-first with light optimisation — selects least-played player groups, applies partner/opponent variety, grade balance, group cooldown, mixed rotation), ROTATION mode (simple fast queue — picks 4 least-played, deterministic team assignment 1+4 vs 2+3, avoids immediate repeats, optional winner-stays). Shared safeguards across all modes: atomic assignment, deterministic tiebreakers, fixed pair enforcement, opponent diversity tracking. Live admin mode selector with mode-specific settings panels. Settings type: `MatchmakingMode = "ADVANCED" | "HYBRID" | "ROTATION"`. New settings: `matchmakingMode`, `hybridGroupSize`, `hybridGroupCooldown`, `hybridGradeSpreadLimit`, `rotationWinnerStays`. All settings configurable via admin UI at `/admin/match-engine-settings`.
...
- **Player Skills Analytics System**: League and Premium player skill tracking with enrollment management, per-player editable skill profiles (slider + star rating + coach comments + priority toggle), radar/bar charts, weak/strong skill identification, bulk skill progress recording, and audit history. Tables: `playerAnalyticsEnrollments`, `playerSkillProgress`, `playerSkillProgressHistory`. Dashboard at `/coach/player-skills`, per-player editing at `/coach/player-skills/:playerId`.
- **Tournaments Module**: Full tournament management with esports-style UI. Supports CLUB/OPEN/LEAGUE/FRIENDLY tournament types. Features: registration system (individual/pair with player pool and pair requests), group+knockout bracket logic with interactive bracket visualization, score submission with auto-advance, standings tables, admin panel (registration approval, payment confirmation, waitlist management, category CRUD, tournament lock). Tables: `tournament_registrations`, `tournament_pair_requests`, `tournament_waitlist`, `tournament_prizes`. Extended `tournaments` table with `bannerUrl`, `maxPlayers`, `skillLevelMin/Max`, `registrationDeadline`, `location`, `socialLinks`, `isLocked`, `entryFee`, `externalEntryFee`, `prizeInfo`, `rules`, `allowedClubIds`. Dual entry fee system: separate fees for club members (internal) vs external players, with per-player fee calculation in finance dashboard based on club membership. Tournament registrations include `paymentStatus` (UNPAID/PENDING/PAID), `paymentMethod`, `paidAt` fields. Routes: `/tournaments` (list), `/tournaments/:id` (detail with 7 tabs: Overview, Players, Pairs, Sign-Up, Matches/Brackets, Prizes, Admin). Admin panel has 6 sub-views: registrations, pairs, waitlist, finance, prizes, settings. Tournament Finance Dashboard shows entry fee KPIs, expected/collected revenue, collection rate, per-player payment management. Prizes system allows admin to create prizes with title, description, placement, value, icon type, and category association. Player-facing Prizes tab with hero banner and prize cards. Payment confirmation sends in-app notifications + chat messages to admins. Weekly payment reminders sent every Monday for unpaid tournament registrations. Demo player seeding for testing (20 dummy players with grades). Tournament Visibility: OPEN tournaments visible to all; non-OPEN visible to owning club by default; `allowedClubIds` extends visibility to additional clubs. Admin search dialog with alphabetical sort and name/email search. API: `server/tournamentRoutes.ts`. Hooks: `client/src/hooks/use-tournaments.ts`.
