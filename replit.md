...
-   **Session Waiting List & Participant States**: Four-state participant system (Confirmed, Waiting List, Invited, Not Attending) with automatic waiting-list assignment when session is full and auto-promotion of first waiting player when a confirmed spot opens. Session details popup modal (triggered by clicking session title) with tabbed view of all participant states and admin controls to move players between states. Per-session finance panel with quick payment toggle, payment method selection (Cash, Online, Card, Bank Transfer, Membership Credit), and payment notes per participant.
