...
- **Club-Restricted Wallet System**: God Mode wallet management with global or club-restricted credit wallets per user. Admins can create wallets, add/remove funds, assign club permissions (multi-select), and view full transaction audit logs. Supports multiple wallets per user with low-balance alerts. Deductions validate against club permissions, and balances are reflected across the app via `/api/my-wallets` and `/api/my-wallets/eligible/:clubId` endpoints.
