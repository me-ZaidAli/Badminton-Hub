...
import { Vote, Plus, Trash2, Eye, BarChart3, Users, Check, X, Loader2, Search, Power, ChevronLeft, Calendar } from "lucide-react";
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 max-w-7xl">
      ...
      <div className="flex items-start justify-between gap-3 mb-6 flex-wrap">
        ...
        <Button onClick={() => setCreateOpen(true)} className="bg-gradient-to-r from-fuchsia-500 to-violet-600 hover:from-fuchsia-400 hover:to-violet-500 text-white" data-testid="button-create-poll">
                ...
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {p.expiresAt && new Date(p.expiresAt) < new Date() && <Badge variant="destructive">Expired</Badge>}
                    </div>
...
function CreateDialog({ open, onClose, targetable }: { open: boolean; onClose: () => void; targetable: { canTargetAll: boolean; clubs: Array<{ id: number; name: string }> } }) {
  const { toast } = useToast();
  const [title, setTitle] = useState("");
  const [question, setQuestion] = useState("");
  const [options, setOptions] = useState<string[]>(["", ""]);
  const [allowMultiple, setAllowMultiple] = useState(false);
  const [audience, setAudience] = useState<"ALL" | "SELECTED">(targetable.canTargetAll ? "ALL" : "SELECTED");
  const [targetClubIds, setTargetClubIds] = useState<number[]>([]);
  const [expiresAt, setExpiresAt] = useState("");

  const create = useMutation({
    mutationFn: async () => apiRequest("POST", "/api/admin/custom-polls", {
      title: title.trim(),
  ...
  const canSubmit = title.trim() && question.trim() && validOptions >= 2 && (audience === "ALL" || targetClubIds.length > 0);

  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Plus className="w-5 h-5" /> Create poll</DialogTitle>
        </DialogHeader>
          ...
          <div className="rounded-lg border p-3">
            ...
            {audience === "SELECTED" && (
              <div className="flex flex-wrap gap-1.5 max-h-40 overflow-y-auto">
                {targetable.clubs.length === 0 ? (
                  <p className="text-xs text-muted-foreground">No clubs available to target.</p>
                ) : targetable.clubs.map(c => {
                  const on = targetClubIds.includes(c.id);
                  return (
                    <button
                      key={c.id}
                      type="button"
                      onClick={() => setTargetClubIds(on ? targetClubIds.filter(x => x !== c.id) : [...targetClubIds, c.id])}
        ...
        <DialogFooter>
          ...
          <Button onClick={() => create.mutate()} disabled={!canSubmit || create.isPending} className="bg-gradient-to-r from-fuchsia-500 to-violet-600 text-white" data-testid="button-submit-create">
