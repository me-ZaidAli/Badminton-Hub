...
- **Advance MUST pre-plan the next stage for auto-placement modes.** After creating the next stage's groups+entries, RANDOMISE/HIERARCHICAL/DESTRUCTION also generate the per-group round-robin PLANNED matches (same C(n,2) loop as stage `restart`/`auto-generate-matches`) so the new stage arrives ready to Start exactly like the first. MANUAL deliberately creates NO matches — teams land in the tray and the organiser places them, then uses per-group auto-generate.
  **Why:** advancing that only seeded groups but left them empty was a behaviour gap — organisers expect a playable stage, not a manual rebuild every round.
