...
export function registerBslRoutes(app: Express) {
  ...
  app.get("/api/bsl/standings", async (req, res) => {
    try {
      ...
      // Aggregate by club — POINTS = sum of rally points per set (matches the
      // Match Summary panel), NOT league standings points. Played/won/lost is
      // counted per rubber to keep P + +/- intuitive (rubber wins minus losses).
      type Row = {
        id: number; name: string; clubName: string; clubLogo: string | null; division: string;
        played: number; won: number; lost: number;
        rubbersFor: number; rubbersAgainst: number; points: number; pointsAgainst: number;
        rubberDiff: number; categories: string[];
      };
      const byClub = new Map<number, Row>();
      // Build set of clubs in scope: any club whose primary OR additional
      // division OR any team's division matches the filter.
      ...
      const ensure = (clubId: number): Row | null => {
        ...
        if (!row) {
          row = {
            id: clubId, name: club.name, clubName: club.name,
            clubLogo: club.logoUrl || null,
            division: club.division || "—",
            played: 0, won: 0, lost: 0,
            rubbersFor: 0, rubbersAgainst: 0, points: 0, pointsAgainst: 0,
            rubberDiff: 0, categories: [],
          };
          byClub.set(clubId, row);
        }
        return row;
      };
      // Seed every (filtered) club + collect category list from teams.
      for (const t of teams) {
      ...
      for (const r of finished) {
        ...
        if (homeClubId != null) {
          ...
          if (row) {
            row.played++;
            row.rubbersFor += rp.homeSetsWon; row.rubbersAgainst += rp.awaySetsWon;
            row.points += rp.home; row.pointsAgainst += rp.away;
        ...
        if (awayClubId != null) {
          ...
          if (row) {
            row.played++;
            row.rubbersFor += rp.awaySetsWon; row.rubbersAgainst += rp.homeSetsWon;
            row.points += rp.away; row.pointsAgainst += rp.home;
            if (awayWon) row.won++; else if (homeWon) row.lost++;
          }
        }
      }
      const enriched = Array.from(byClub.values()).map(r => ({ ...r, rubberDiff: r.points - r.pointsAgainst }));
      enriched.sort((a, b) => b.points - a.points || b.rubberDiff - a.rubberDiff || b.played - a.played || a.name.localeCompare(b.name));
  ...
  // Helper — rally points for a single rubber. Mirrors the Match Summary
  // (QuickResults.tsx ~line 240): sum each side's per-set scores from
  // rubber.setScores; fall back to (homeScore, awayScore) treated as one set
  // when no multi-set breakdown exists. This is the SAME number admins see in
  // the "Points 349 vs 355" panel — leaderboards must agree with it.
  function rubberRallyPoints(r: any): { home: number; away: number; homeSetsWon: number; awaySetsWon: number } {
  ...
  async function loadResolvedPlayers() {
    ...
    const userIds = Array.from(new Set(players.map(p => p.userId).filter((x): x is number => x != null)));
    ...
    return players.map(p => ({
      ...p,
      resolvedName: (uMap.get(p.userId)?.fullName) || p.displayName || `Player #${p.id}`,
    }));
  }

  // Shared player-leaderboard computation (rally points per player across all
  // FINISHED rubbers). Used by the leaderboard list AND the single-player stats
  // endpoint so the player card never depends on the full-list fetch.
  type PlayerLbRow = {
    ...
    setsFor: number; setsAgainst: number; winRate: number; points: number; position: number;
  };
  async function computePlayerLeaderboard(division?: string): Promise<PlayerLbRow[]> {
    ...
    const ensure = (p: any) => {
      ...
      if (!row) {
        ...
        row = {
          playerId: p.id, fullName: p.resolvedName,
    ...
    for (const r of finished) {
      ...
      for (const pid of homePlayers) {
        ...
        if (!row) continue;
        row.matchesPlayed++;
        row.setsFor += rp.homeSetsWon; row.setsAgainst += rp.awaySetsWon;
        row.points += rp.home;
        if (homeWon) row.won++; else if (awayWon) row.lost++;
      }
      for (const pid of awayPlayers) {
        ...
        if (!row) continue;
        row.matchesPlayed++;
        row.setsFor += rp.awaySetsWon; row.setsAgainst += rp.homeSetsWon;
        row.points += rp.away;
        if (awayWon) row.won++; else if (homeWon) row.lost++;
      }
    }
  ...
  app.get("/api/bsl/player-leaderboard", async (req, res) => {
    try {
      const division = req.query.division as string | undefined;
      res.json(await computePlayerLeaderboard(division));
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  // Single-player stats card. Self-sufficient — computes the full leaderboard
  // and returns just this player's row (with global rank), so the battlecard
  // shows real points even if the full-list query isn't loaded.
  app.get("/api/bsl/players/:id/stats", async (req, res) => {
    try {
      const id = Number(req.params.id);
      if (!Number.isFinite(id)) return res.status(400).json({ message: "Bad id" });
      const rows = await computePlayerLeaderboard();
      const found = rows.find(r => r.playerId === id);
      if (!found) return res.status(404).json({ message: "No stats for this player yet" });
      res.json(found);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  // === CLUB LEADERBOARD ===
  // Per-club aggregate of FINISHED rubbers: rubbers won/lost, sets won/lost,
  // and POINTS = sum of every set's rally score on that club's side (the same
  // "Points" the Match Summary shows, e.g. 349 vs 355). Club attribution is
  // immutable via rubber.homeTeamId → bslTeams.bslClubId, falling back to the
  // fixture's home/away club so transfers don't rewrite history.
    ...
    try {
      ...
      for (const c of clubs) {
        byClub.set(c.id, {
          ...
          played: 0, won: 0, lost: 0,
          setsFor: 0, setsAgainst: 0, points: 0, pointsAgainst: 0,
          winRate: 0, playerCount: 0,
        });
      }
  ...
  // === PAIR LEADERBOARD ===
  // Per doubles-pair (homePlayer1+homePlayer2 sorted). Each unordered pair
  // collected across every FINISHED rubber where both player IDs are set.
  app.get("/api/bsl/pair-leaderboard", async (req, res) => {
    try {
      const division = req.query.division as string | undefined;
      const finished = await db.select().from(bslRubbers).where(eq(bslRubbers.status, "FINISHED" as any));
      const players = await loadResolvedPlayers();
      ...
      let rows = Array.from(byPair.values()).map(r => ({
        ...r, winRate: r.matchesPlayed > 0 ? Math.round((r.won / r.matchesPlayed) * 100) : 0,
      }));
      if (division) rows = rows.filter(r => r.division === division);
      rows.sort((a, b) =>
        b.points - a.points ||
  ...
  // Returns every FINISHED rubber for the given entity, with player + club
  // names + fixture date, so the leaderboard click-through can render full
  // match list + sparkline trend.
  app.get("/api/bsl/entity-matches", async (req, res) => {
    try {
      const kind = String(req.query.kind || ""); // "player" | "club" | "pair"
      const id = req.query.id ? Number(req.query.id) : null;
      const id2 = req.query.id2 ? Number(req.query.id2) : null; // pair partner
      const finished = await db.select().from(bslRubbers).where(eq(bslRubbers.status, "FINISHED" as any));
      const players = await loadResolvedPlayers();
      const clubs = await db.select().from(bslClubs);
      const fixtures = await db.select().from(bslFixtures);
      ...
      // Immutable side→club resolution: rubber.homeTeamId → team.bslClubId,
      // falling back to fixture.homeClubId. Avoids historical drift on player
      // transfers.
      const sideClub = (teamId: number | null, fixtureClubId: number | null | undefined): number | null => {
        if (teamId != null) {
          const t = tMap.get(teamId);
      ...
      const out: Match[] = [];
      for (const r of finished) {
        const hps = [r.homePlayer1Id, r.homePlayer2Id].filter((x): x is number => x != null);
        const aps = [r.awayPlayer1Id, r.awayPlayer2Id].filter((x): x is number => x != null);
        const fx = fMap.get(r.bslFixtureId);
        const homeClubId = sideClub(r.homeTeamId ?? null, fx?.homeClubId);
        const awayClubId = sideClub(r.awayTeamId ?? null, fx?.awayClubId);
  ...
  // names. Powers the big "battle" announcement card on the BSL main page.
  app.get("/api/bsl/fixtures-showcase", async (req, res) => {
    try {
      const limit = Math.max(1, Math.min(12, Number(req.query.limit) || 6));
      // LIVE first, then SCHEDULED / WARMUP soonest-first.
      const all = await db.select().from(bslFixtures);
      const eligible = all.filter(f => ["LIVE", "WARMUP", "SCHEDULED"].includes(String(f.status || "")));
      eligible.sort((a, b) => {
      ...
      if (picked.length === 0) return res.json([]);
      const fixtureIds = picked.map(f => f.id);
      const clubIds = Array.from(new Set(picked.flatMap(f => [f.homeClubId, f.awayClubId]).filter((x): x is number => x != null)));
      const [rubbers, clubs] = await Promise.all([
        db.select().from(bslRubbers).where(inArray(bslRubbers.bslFixtureId, fixtureIds)).orderBy(bslRubbers.rubberNumber),
        clubIds.length ? db.select().from(bslClubs).where(inArray(bslClubs.id, clubIds)) : Promise.resolve([] as any[]),
      ]);
      const cMap = new Map(clubs.map(c => [c.id, c]));
      const teamIds = Array.from(new Set(rubbers.flatMap(r => [r.homeTeamId, r.awayTeamId]).filter((x): x is number => x != null)));
      const teams = teamIds.length ? await db.select().from(bslTeams).where(inArray(bslTeams.id, teamIds)) : [];
      const tMap = new Map(teams.map(t => [t.id, t]));
      ...
      const playerMap = new Map(players.map(p => [p.id, {
        id: p.id,
        name: p.displayName || (p.userId != null ? userMap.get(p.userId)?.name : null) || `Player #${p.id}`,
      }]));
      const teamWithMembers = (teamId: number | null) => {
        ...
        return {
          ...
          name: t.name,
          category: t.category,
          members: memberRows.filter(m => m.bslTeamId === t.id).map(m => playerMap.get(m.bslPlayerId)).filter(Boolean),
        };
      };
  ...
  app.get("/api/bsl/clubs/:id/teams", requireAuth, async (req, res) => {
    try {
      ...
      // Real per-player stats from FINISHED rubbers (same rally-point maths as
      // the player leaderboard). Computed here so the club dashboard squad shows
      // real points without depending on a second client-side fetch/merge.
      const finishedRubbers = await db.select().from(bslRubbers).where(eq(bslRubbers.status, "FINISHED" as any));
      const statByPlayer = new Map<number, { points: number; matchesPlayed: number; won: number }>();
      const ensureStat = (pid: number) => {
        let s = statByPlayer.get(pid);
        if (!s) { s = { points: 0, matchesPlayed: 0, won: 0 }; statByPlayer.set(pid, s); }
        return s;
      };
      for (const r of finishedRubbers) {
        const rp = rubberRallyPoints(r);
        const homeWon = rp.homeSetsWon > rp.awaySetsWon;
        const awayWon = rp.awaySetsWon > rp.homeSetsWon;
        for (const pid of [r.homePlayer1Id, r.homePlayer2Id].filter((x): x is number => x != null)) {
          const s = ensureStat(pid); s.matchesPlayed++; s.points += rp.home; if (homeWon) s.won++;
        }
        for (const pid of [r.awayPlayer1Id, r.awayPlayer2Id].filter((x): x is number => x != null)) {
          const s = ensureStat(pid); s.matchesPlayed++; s.points += rp.away; if (awayWon) s.won++;
        }
      }
      const statsFor = (pid: number) => {
        const s = statByPlayer.get(pid);
        return {
          points: s?.points ?? 0,
          matchesPlayed: s?.matchesPlayed ?? 0,
          won: s?.won ?? 0,
          winRate: s && s.matchesPlayed > 0 ? Math.round((s.won / s.matchesPlayed) * 100) : 0,
        };
      };
      const membersByTeam = new Map<number, string[]>();
      const memberObjsByTeam = new Map<number, Array<{ playerId: number; name: string; points: number; matchesPlayed: number; won: number; winRate: number }>>();
      for (const m of members) {
        ...
        if (name) {
          arr.push(name);
          objs.push({ playerId: m.bslPlayerId, name, ...statsFor(m.bslPlayerId) });
        }
        membersByTeam.set(m.bslTeamId, arr);
  ...
  app.patch("/api/bsl/clubs/:clubId/players/:playerId", requireAuth, async (req, res) => {
    try {
      ...
      if (typeof req.body.division === "string") {
        const requested = req.body.division.trim();
        ...
        if (!joined.has(requested)) {
          return res.status(400).json({ message: `Club doesn't play in "${requested}". Available: ${[...joined].join(", ")}.` });
        }
        const current = p.division || club.division;
        if (requested !== current) {
          const memberships = await db.select().from(bslTeamMembers).where(eq(bslTeamMembers.bslPlayerId, playerId));
          if (memberships.length > 0) {
            return res.status(409).json({ message: "Player is already in a pair. Remove them from their pair(s) before switching divisions." });
          }
        }
  ...
  // Update display name + bio on /api/bsl/players/me. Players cannot change
  // their own division — that's a club-admin action via
  // PATCH /api/bsl/clubs/:clubId/players/:playerId.
